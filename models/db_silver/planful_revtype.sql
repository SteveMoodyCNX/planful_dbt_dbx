{{ 
  config(
    ...
    unique_key = ['_revtype_key','_tenant_name'],
    incremental_strategy = 'merge',
	on_schema_change = 'sync_all_columns',
    options = {"mergeSchema": "true"},
    schema = 'db_silver',
    comment = 'Silver table for Planful revenue type hierarchy with CDC tracking and flattened rollup levels'
  ) 
}}

WITH source_data_raw AS (
	SELECT 
		MemberId AS `Revenue_Type_Leaf_Line_ID`,
		Code AS `Revenue_Type_Leaf_ID`,
		Name AS `Revenue_Type`,
		SHA2(CONCAT_WS('|', 
		    COALESCE(Code, ''),
		    COALESCE(Name, '')
		), 256) AS `_revtype_key`,
		...
		{{ generate_rollup_columns('Revenue_Type', 2, table_alias='t', include_hash=False) }},
		_source_loaded_at,
		_source_file,
		_tenant_name
FROM {{ source('db_bronze', 'planful_revtype') }} t 
),

source_data AS (
  SELECT * EXCEPT(Sort)
  FROM source_data_raw
  QUALIFY ROW_NUMBER() OVER (PARTITION BY _revtype_key, _tenant_name ORDER BY Sort ASC) = 1
),
...
source_with_metadata AS (
  SELECT 
    *,
    ...
    MD5(CONCAT_WS('||', 
      COALESCE(CAST(Revenue_Type_Leaf_Line_ID AS STRING), ''),
...
records_to_delete AS (
  SELECT 
    t.Revenue_Type_Leaf_Line_ID,
    t.Revenue_Type_Leaf_ID,
    t.Revenue_Type,
    t._revtype_key,
    t.RollupOperator,
    t.MemberType,
  ...
  LEFT JOIN source_data s 
    ON t._revtype_key = s._revtype_key
    AND t._tenant_name = s._tenant_name
  WHERE s._revtype_key IS NULL
